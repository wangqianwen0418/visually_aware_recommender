...
workdir = '/home/qianwen/KDD/cfree/ckpt/promoted_pretrained_50_dropout{}_{}'.format(args.dropout, args.kernel_wd)
...
class Siamese(torch.nn.Module):
    def __init__(self, latent_d, batch_size, usernum):
        ...
        if args.dropout:
            net.classifier = torch.nn.Sequential(net.classifier[1], net.classifier[2], torch.nn.Dropout(0.5), net.classifier[4], net.classifier[5], torch.nn.Linear(4096, latent_d), torch.nn.ReLU(inplace=True), torch.nn.Dropout(0.5))
        else:
            net.classifier = torch.nn.Sequential(net.classifier[1], net.classifier[2], net.classifier[4], net.classifier[5], torch.nn.Linear(4096, latent_d), torch.nn.ReLU(inplace=True))
        self.net = net
...
for name, param in siamese_net.named_parameters():
    if name == 'kernel':
        params.append({'params': [param], 'weight_decay': args.kernel_wd, 'lr': 1e-2})
    else:
